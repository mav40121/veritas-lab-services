...
function pdfFooterSections(doc: jsPDF, study: Study, instrumentNames: string[], y: number, pw: number, margin: number, contentW: number) {
  ...
  const cliaP = (study.cliaAllowableError*100).toFixed(1);
  // Map TEa to CFR citation — covers most common CLIA-regulated analytes
  const cfrMap: { [key: string]: string } = {
    "7.5": "42 CFR §493.931",  // Creatinine
    "10.0": "42 CFR §493.931", // Glucose, TSH
    "7.0": "42 CFR §493.941",  // Hemoglobin
    "4.0": "42 CFR §493.931",  // Sodium
    "5.0": "42 CFR §493.931",  // Potassium
  };
  ...
  const specs = [
    ...
    ["CLIA Total Allowable Error", `±${cliaP}%  (${cfrCitation})`],
  ...
  for (let i=0;i<Math.max(specs.length,supporting.length);i++) {
    if (specs[i]) {
      ...
      // Highlight TEa value in teal and make it a hyperlink
      if (i === 1) {
        setRgb(doc, [0, 120, 130]);
        doc.setFont("helvetica","bold");
        doc.text(specs[i][1], margin+halfW-2, y, { align: "right" });
        // Add clickable link over the CFR citation text
        const textW = doc.getTextWidth(specs[i][1]);
        doc.link(margin+halfW-2-textW, y-3, textW, 4.5, { url: cfrUrl });
