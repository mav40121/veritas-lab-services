...
function clientInstrumentDisplayMultiline(study: Study): string[] {
  try {
    if ((study as any).instrumentMeta) {
      const meta = typeof (study as any).instrumentMeta === "string" ? JSON.parse((study as any).instrumentMeta) : (study as any).instrumentMeta;
      const entry = meta["0"];
      if (entry) {
        const line1 = entry.nickname ? `${entry.model} (${entry.nickname})` : entry.model;
        if (entry.serial_number) return [line1, `S/N: ${entry.serial_number}`];
        return [line1];
      }
    }
  } catch {}
...
function pdfHeader(doc: jsPDF, study: Study, pw: number, margin: number): number {
  ...
  doc.text(`Instrument: ${clientInstrumentDisplayInline(study)}`, pw-margin, y+5, { align: "right" });
  // Title on its own line below
  y += 10; hLine(doc, y); y += 5;
  const typeLabel = study.studyType === "cal_ver" ? "Calibration Verification (CLSI EP06)" : "Correlation / Method Comparison";
...
function pdfSupportingPage(doc: jsPDF, study: Study, instrumentNames: string[], pw: number, margin: number, contentW: number) {
  ...
  const specs = [
    ["Study Type", study.studyType === "cal_ver" ? "Calibration Verification (CLSI EP06)" : "Correlation / Method Comparison"],
  ...
  const instLines = clientInstrumentDisplayMultiline(study);
  const supporting = [
    ["Analyst", study.analyst],
    ["Date", study.date],
    ["Instrument(s)", instLines.join("\n")],
