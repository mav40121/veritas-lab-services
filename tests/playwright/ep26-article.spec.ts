...
//
// Gate 3 step 8 receipt for the EP26-A reagent lot verification cornerstone
...
test.describe("EP26-A reagent lot verification article", () => {
  test("renders hero, protocol, FAQ, and the VeritaCheck CTA", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: /EP26-A Reagent Lot Verification/i })
    ).toBeVisible({ timeout: 15000 });
    await expect(page.getByText(/The EP26-A protocol, step by step/i)).toBeVisible();
