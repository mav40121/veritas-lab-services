...
## Copy Rules
- NO em dashes (—) in public-facing artifacts. Use commas, colons, or plain hyphens (-) instead. Public-facing means anything a customer, prospect, accreditor, or non-employee sees: website copy, leave-behinds, brochures, generated PDFs (VeritaCheck, VeritaComp, VeritaScan, VeritaMap, VeritaLab, certificates, CMS 209), demo assets, marketing emails, social cards, externally shown slide decks, the master citation index xlsx, and book content. Internal-only files where em-dashes are acceptable: STANDING_REQUIREMENTS.md, SESSION_HANDOFF files, skill files, code comments, agent-to-user chat, scratch notes. When in doubt, treat as public-facing.
...
5. **Each About tab is hand-written for its product.** The About sheet's product description, how-to-use, disclaimer, and source/provenance language are written specifically for the module that produced the workbook. There is no shared helper or template. The structural boilerplate that IS identical across products (brand color constants, page-setup header/footer pattern, sheet-protection options block) may be copy-pasted across routes, but the About content itself must be authored per product so it actually describes that product accurately.
...
- **SOURCE OF TRUTH RULE**: Before stating how the code, production, or database behaves, read the actual artifact this turn (the current file, the live API response, the real schema), not memory, a conversation summary, a prior session, or a tool's cached index. A glob or search that returns "not found" is not proof of absence: confirm by reading the path (globs run relative to the tool's working directory, which may not be the repo). If you are recalling something, re-verify it against the source before acting. Validate SQL against a REAL schema snapshot (an /api/admin/backup-db download or a long-lived dev veritas.db) via scripts/audit-sql-columns.mjs, never a from-scratch db.ts boot, which yields an incomplete schema. Added 2026-07-09 after an audit reported scripts/audit-sql-columns.mjs as missing based on a glob run against the wrong directory; the script existed and was correct.
- **SCRIPT-WITH-ARTIFACT RULE**: Any work product generated by Python (xlsx, pdf, csv, docx, pptx, json, or any other artifact built via code) must be delivered together with the .py script that produced it, in the same response. Sharing only the output file is a procedural breach. This applies to one-off builds, multi-pass pipelines, and audit/QA scripts. If multiple scripts produced the artifact (e.g., a trim pass plus a builder), share all of them. The user is the regulatory and operational authority on the work product, the script is how the agent shows its math. Established 2026-05-01 after the v0.9 citation index ship, where the agent delivered the xlsx and prose feedback but withheld the three scripts (`apply_trim_v06.py`, `range_compress_v06.py`, `build_xlsx_v09.py`) that backed every claim in the feedback.
