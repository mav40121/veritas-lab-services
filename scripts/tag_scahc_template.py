...
def main():
    ...
    # (c) Inject a {references} placeholder paragraph immediately after the
    #     REFERENCES/ATTACHMENTS/APPENDIX heading. The original SCAHC template
    #     leaves the section blank for a human to fill. We fill it from each
    #     policy's cfr_text_blocks + accreditor crosswalk via docxtemplater.
    #     The new paragraph uses the same indented style (left=720) so it
    #     visually flows under the heading.
    ref_heading_pattern = re.compile(
        r'(<w:t>REFERENCES/ATTACHMENTS/APPENDIX</w:t></w:r></w:p>)'
    )
    ref_para_xml = (
        '<w:p w14:paraId="6E465E5C" w14:textId="77777777" '
        'w:rsidR="00976EA2" w:rsidRDefault="00976EA2" w:rsidP="00976EA2">'
        '<w:pPr><w:ind w:left="720"/></w:pPr>'
        '<w:r><w:t xml:space="preserve">{references}</w:t></w:r>'
        '</w:p>'
    )
    ref_count = len(ref_heading_pattern.findall(new_doc_xml))
    new_doc_xml = ref_heading_pattern.sub(r'\1' + ref_para_xml, new_doc_xml)
    results.append((f"inject {{references}} placeholder after heading ({ref_count} match)", ref_count > 0))
