...
function staffingGridFte(lines, hoursPerFteYear = 2080) {
  ...
  const weeklyHours = lines.reduce((s, l) => s + ((l.hoursPerShift || 0) * (l.daysPerWeek || 0)), 0);
...
// LTSHealth Staffing Grid, Main Lab (hoursPerShift, daysPerWeek), from the tool's
// sheet 2 (TOTAL = hours x days). The original sheet's over/under column was retired
// 2026-08-20; its values netted to zero across these lines, so the 1,481 hr / 37.0 FTE
// totals are identical with or without it.
const LTS = [
  [8, 5], [8, 5], [8, 5], [8, 5], [8, 5], [28, 5], [8, 5], [8, 5],
  [12, 6], [12, 2], [8, 5], [8, 2], [4, 5], [8, 5], [8, 2], [12, 5], [8, 2],
  [24, 5], [24, 2], [24, 5], [16, 2], [26, 5], [12, 2], [6, 5], [16, 1], [37, 5], [26, 2],
].map(([h, d]) => ({ hoursPerShift: h, daysPerWeek: d }));
...
check("base line (8x5 = 40 hr)", staffingGridFte([{ hoursPerShift: 8, daysPerWeek: 5 }]).weeklyHours === 40);
